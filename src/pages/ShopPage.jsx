...
function ShopPage() {
  ...
  const [filters, setFilters] = useState({ type: [], priceRanges: [], priceMin: '', priceMax: '', brands: [] });
  ...
  const handleClearFilters = () => setFilters({ type: [], priceRanges: [], priceMin: '', priceMax: '', brands: [] });

  const handleTryVR = async () => { await new Promise(r => setTimeout(r, 900)); };
  const handleBuyNow = async () => { await new Promise(r => setTimeout(r, 1000)); };

  return (
    <>
      <Navbar brand="Nazario" />
      <div className="bg-transparent py-2 mb-2"> {/* subheader no longer needs sticky since navbar is fixed */}
    <div className="container d-flex flex-wrap align-items-center justify-content-between gap-2">
     <h1 className="m-0 fw-bold" style={{ fontSize: 'clamp(22px,2.6vw,28px)' }}>Shop All</h1>
      <div className="d-flex align-items-center gap-2">
            <button className="btn btn-light border rounded-pill d-flex align-items-center gap-2 px-3"
                    onClick={()=> setFilterOpen(true)}>
              <i className="bi bi-sliders2" /><span>Filters</span>
            </button>
            <div className="dropdown">
              <button className="btn btn-light border rounded-pill d-flex align-items-center gap-2 px-3 dropdown-toggle"
